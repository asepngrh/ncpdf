...
export async function POST(req: NextRequest) {
  try {
    ...
    const xlsxBytes = await convertPdfToExcel(arrayBuffer);

    const outputName = `${getBaseFileName(filename)}.xlsx`;

    return new NextResponse(xlsxBytes as unknown as BodyInit, {
      status: 200,
      headers: {
