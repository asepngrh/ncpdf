...
export default function WordToPdfPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || "Gagal mengonversi file Word ke PDF.");
      }
  ...
  return (
    <ToolLayout
      title="Word ke PDF"
      description="Konversi dokumen Word (DOC, DOCX, ODT) ke format PDF berkualitas tinggi dengan format dan tata letak yang presisi."
      category="Convert"
      isClientSide={false}
    >
      <div className="space-y-6">
        ...
        {!file && (
          <FileDropzone
            ...
            title="Pilih atau drop file Word (.docx, .doc, .odt)"
          />
        )}
            ...
            <CardContent className="p-6 space-y-5">
              ...
              {isProcessing && (
                <div className="pt-2">
                  <ProgressBar
                    progress={progress}
                    label="Mengonversi Dokumen Word ke PDF..."
                    sublabel="Mohon tunggu sejenak, format dan tata letak sedang diproses..."
                  />
                </div>
              )}
